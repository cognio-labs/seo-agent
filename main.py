...
if 'completed_nodes' not in st.session_state:
    # Track completed workflow nodes to calculate accurate progress
...
if st.session_state.workflow_running and initial_query:
    with results_container:
        ...
        # Placeholders to avoid content duplication on each iteration
        tab1_placeholder = tab1.empty()
        tab2_placeholder = tab2.empty()
        tab3_placeholder = tab3.empty()
        tab4_placeholder = tab4.empty()
        tab5_placeholder = tab5.empty()
        ...
        try:
            ...
            for state in run_full_workflow(initial_query, user_preferences):
                # Update current node and completed nodes set
                for key in state.keys():
                    if key not in st.session_state.completed_nodes:
                        st.session_state.current_node = key
                        st.session_state.completed_nodes.add(key)
                        # Preserve node outputs for status view
                        st.session_state.node_outputs[key] = state.get(key)
                        completed_count = len(st.session_state.completed_nodes)
                        break

                # Update progress and ensure it never exceeds 1.0
                progress = min(completed_count / total_nodes, 1.0)
                ...
                # Update tabs with results (clearing previous content each iteration)

                # ---------------- Tab 1 ----------------
                tab1_placeholder.empty()
                ...
                # ---------------- Tab 3 ----------------
                tab3_placeholder.empty()
                with tab3_placeholder.container():
                ...
                # ---------------- Tab 4 ----------------
                tab4_placeholder.empty()
                with tab4_placeholder.container():
                    if 'content_briefs' in state and state['content_briefs']:
                        ...
                        for i, brief in enumerate(state['content_briefs']):
                            with st.expander(
                                ...
                                st.json(brief)
                # ---------------- Tab 5 ----------------
                tab5_placeholder.empty()
                with tab5_placeholder.container():
