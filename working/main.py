...
with col2:
    ...
    for node_id, node_name in workflow_nodes:
        if node_id in st.session_state.completed_nodes:
            st.markdown(
    ...
    with results_container:
        ...
        try:
            ...
            for state in run_full_workflow(initial_query, user_preferences):
                # Update current node based on workflow nodes
                for node_id, node_name in workflow_nodes:
                    if node_id in state and node_id not in st.session_state.completed_nodes:
                        st.session_state.current_node = node_id
                        st.session_state.completed_nodes.add(node_id)
                        completed_count = len(st.session_state.completed_nodes)
                        break

                # Store all state data
                st.session_state.node_outputs.update(state)
            ...
            # Save final results
            st.session_state.workflow_results = st.session_state.node_outputs
            st.session_state.workflow_running = False
