...
def entity_semantic_optimization_node(
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system",
         "You are an SEO expert specializing in entity extraction and semantic keyword optimization. Analyze the competitor content and extract key entities, semantic keywords, topic clusters, and content gaps. Respond with a JSON object containing: entities, semantic_keywords, topic_clusters, and content_gaps."),
...
def create_article_blueprint_node(state: FullWorkflowState) -> Dict[str, Any]:
    ...
    # Check if this is a revision (iteration_count > 0)
    current_iteration = state.get('iteration_count', 0)
    if state.get('verification_feedback'):
        # This is a revision, so increment the iteration count
        current_iteration += 1
        print(f"Creating revised blueprint (iteration {current_iteration}) for: {content_brief.get('topic', 'Unknown')}")
    else:
        print(f"Creating blueprint for: {content_brief.get('topic', 'Unknown')}")

    prompt = ChatPromptTemplate.from_messages([
        ("system",
         "You are a senior content strategist creating article blueprints for SEO-optimized content. Create a comprehensive article blueprint with structure, headings, word counts, keyword placement, internal linking opportunities, and call-to-action placement. Respond with a JSON object containing the complete blueprint."),
...
def write_seo_article_node(state: FullWorkflowState) -> Dict[str, Any]:
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system",
         "You are an expert SEO content writer. Write a comprehensive, engaging, and SEO-optimized article following the blueprint. Use proper markdown formatting, incorporate keywords naturally, ensure proper structure, and demonstrate expertise. Return ONLY the markdown formatted article without any additional commentary."),
        ("user", "Blueprint: {blueprint}\n\nContent Brief: {content_brief}\n\nEntities to include: {entities}\n\nSemantic keywords: {semantic_keywords}\n\nTarget audience: {target_audience}\n\nTone of voice: {tone_of_voice}{feedback_context}\n\nWrite the complete SEO-optimized article now.")
    ])
...
def verify_article_node(state: FullWorkflowState) -> Dict[str, Any]:
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system",
         "You are an SEO quality assurance expert. Analyze the article for content quality, keyword usage, on-page optimization, headings structure, and technical requirements. Return a JSON with: approved (true/false), score (0-100), issues (list of specific issues), and strengths (what was done well)."),
...
def media_planner_node(state: FullWorkflowState) -> Dict[str, Any]:
    ...
    # Debug: Check data before creating prompt
    print(f"Debug - Article length: {len(article)}")
    print(f"Debug - Target keyword: {repr(target_keyword)}")
    print(f"Debug - Topic from brief: {repr(content_brief.get('topic', ''))}")

    try:
        # Use a cleaner format to avoid any parsing issues
        system_message = "You are a media planning expert for SEO content. Analyze the article and create a comprehensive media plan. Return a JSON with: youtube_queries (search queries for videos), image_prompts (detailed prompts for image generation), and media_placement (where to place each media element)."
        user_message = """Article content: {article}
...
Create a comprehensive media plan."""
        ...
        # Debug: Check prompt variables
        print(f"Debug - Prompt input variables: {prompt.input_variables}")
    except Exception as e:
    ...
    try:
        ...
        # Debug: Check invoke parameters
        invoke_params = {
            "article": article[:2000],  # Limit for API
            "target_keyword": target_keyword,
            "topic": content_brief.get('topic', '')
        }
        print(f"Debug - Invoke params keys: {list(invoke_params.keys())}")
        print(f"Debug - Article param length: {len(invoke_params['article'])}")
        print(f"Debug - Target keyword param: {repr(invoke_params['target_keyword'])}")
        print(f"Debug - Topic param: {repr(invoke_params['topic'])}")
